...
class Model:
    ...
    def logit(self, data, isTrain=True, dropout=0.7):
        helper.log(self.logger, '[model] Data shape = {0}'.format(data.shape))
        ...
        with tf.variable_scope('conv1') as scope:
            ...
            helper.log(self.logger, '[model] Data shape at conv1 = {0}'.format(data.shape))
        ...
        with tf.variable_scope('conv2') as scope:
            ...
            data = tf.nn.max_pool(data, [1, 3, 3, 1], [1, 2, 2, 1], 'SAME')
            data = self.dropout(data, isTrain, dropout)
            helper.log(self.logger, '[model] Data shape at conv2 = {0}'.format(data.shape))
        ...
        with tf.variable_scope('fc1') as scope:
            ...
            data = self.dropout(data, isTrain, dropout)
            helper.log(self.logger, '[model] Data shape at fc1 = {0}'.format(data.shape))
            
        with tf.variable_scope('fc2') as scope:
            data = self.fc(data, 10, scope.name)
            data = self.dropout(data, isTrain, dropout)
            helper.log(self.logger, '[model] Data shape at fc2 = {0}'.format(data.shape))
